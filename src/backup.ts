...
const fileName = (nowMs: number): string => {
  ...
  return `dreamfeed-backup-${d.getFullYear()}${p(d.getMonth() + 1)}${p(d.getDate())}.json`;
};
...
export async function exportBackup(
  ...
  const now = Date.now();
  const json = serializeBackup(profile, unit, getAllEvents(), now);
  ...
  await Sharing.shareAsync(uri, {
    ...
    dialogTitle: 'Export Dreamfeed backup',
  });
}
